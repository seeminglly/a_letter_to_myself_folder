    ...
    try:
        ...
        connection = pika.BlockingConnection(
            pika.ConnectionParameters(
                host='34.47.117.180',  # ✅ GCP RabbitMQ 외부 IP
                port=5672,
                virtual_host='/',
                credentials=pika.PlainCredentials('myuser', 'mypassword')  # ✅ 실제 설정
            )
        )
        ...
        for letter in letters_data:
            message = {
                ...
                "user_id": letter["user_id"],  # 여기에 user_id 추가
            }
...
def dummy_letters_view(request):
    return Response([
        {"id": 1, "content": "요즘 날씨가 좋아서 기분이 좋다.", "user_id": 1},
        {"id": 2, "content": "오늘은 우울한 하루였다.", "user_id": 1},
        {"id": 3, "content": "시험 공부가 너무 힘들다.", "user_id": 1},
        {"id": 4, "content": "친구와 여행 계획을 짰다.", "user_id": 1},
        {"id": 5, "content": "가족과 함께 보내는 시간은 소중하다.", "user_id": 1},
    ])